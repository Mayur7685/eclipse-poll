 ...
 */
function scalarFromSeed(hexSeed: string): bigint {
...
let SERVER_KEYPAIR: { sk: bigint; pk: JubjubPoint };
...
if (SECRET_HEX && SECRET_HEX.replace(/^0x/, '').length >= 32) {
  ...
  console.warn(
    '[signing] WARNING: ATTESTATION_SECRET_KEY not set. Using ephemeral keypair.\n' +
...
export function getProviderPublicKey(): JubjubPoint {
