...
const CHAINS: Record<number, any> = { 1: mainnet, 8453: base, 42161: arbitrum, 10: optimism, 137: polygon };
...
const ERC20_ABI = [{ name: 'balanceOf', type: 'function', inputs: [{ name: 'account', type: 'address' }], outputs: [{ name: '', type: 'uint256' }], stateMutability: 'view' }] as const;
const ERC721_ABI = [
...
const ERC1155_ABI = [{ name: 'balanceOf', type: 'function', inputs: [{ name: 'account', type: 'address' }, { name: 'id', type: 'uint256' }], outputs: [{ name: '', type: 'uint256' }], stateMutability: 'view' }] as const;
...
export async function checkTokenBalance(
  ...
  minBalance: bigint,
): Promise<{ passed: boolean; balance: string; message: string }> {
  if (!isAddress(walletAddress)) return { passed: false, balance: '0', message: 'Invalid wallet address' };
  if (!isAddress(tokenAddress)) return { passed: false, balance: '0', message: 'Invalid token address' };
...
export async function checkNftOwnership(
  ...
  if (!isAddress(walletAddress)) return { passed: false, balance: '0', message: 'Invalid wallet address' };
  if (!isAddress(contractAddress)) return { passed: false, balance: '0', message: 'Invalid contract address' };
