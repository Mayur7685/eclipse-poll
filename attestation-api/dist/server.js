...
const communitiesStore = loadStore();
...
app.get(['/communities/:id', '/api/communities/:id'], (req, res) => {
...
app.post(['/communities/confirm', '/api/communities/confirm'], (req, res) => {
...
app.post(['/polls/confirm', '/api/polls/confirm'], (req, res) => {
    const poll = {
    ...
    if (targetComm) {
        ...
        const idx = targetComm.polls.findIndex((p) => p.poll_id === poll.poll_id);
        if (idx >= 0) {
 ...
 */
async function checkRequirements(community, evmAddress, connectedAccounts) {
        ...
        for (const req of group.requirements ?? []) {
            ...
            switch (req.type) {
                case 'FREE':
                    ...
                    break;
                case 'ALLOWLIST': {
                    const allowlist = (req.params?.addresses ?? []).map((a) => a.toLowerCase());
                ...
                case 'X_FOLLOW': {
                    ...
                    try {
                        ...
                        if (!apiKey) {
                            passed = false;
                            message = 'TWITTERAPI_IO_KEY not configured';
                            break;
                        }
                        ...
                        const res2 = await (await import('axios')).default.get('https://api.twitterapi.io/twitter/user/check_follow_relationship', { headers: { 'X-API-Key': apiKey }, params: { source_user_name: username.replace('@', ''), target_user_name: target } });
                        passed = res2.data?.data?.following === true;
                        message = passed ? `@${username} follows @${target}` : `@${username} does not follow @${target}`;
                    }
                    catch (e) {
                        passed = false;
                        message = `Twitter check failed: ${e.message}`;
                    }
                    break;
                }
                case 'DISCORD_MEMBER': {
                    ...
                    try {
                        ...
                        if (userToken) {
                            ...
                            passed = r.data.some(g => g.id === serverId);
                        }
                        ...
                        passed = false;
                        message = `Discord check failed: ${e.message}`;
                    }
                    break;
                }
                case 'DISCORD_ROLE': {
                    ...
                    try {
                        const r = await (await import('axios')).default.get(`https://discord.com/api/v10/guilds/${req.params?.serverId}/members/${discordId}`, { headers: { Authorization: `Bot ${process.env.DISCORD_BOT_TOKEN}` }, validateStatus: null });
                        passed = r.status === 200 && (r.data?.roles ?? []).includes(req.params?.roleId ?? '');
                        message = passed ? `Has required Discord role` : `Missing required Discord role`;
                    }
                    catch (e) {
                        passed = false;
                        message = `Discord role check failed: ${e.message}`;
                    }
                    break;
                }
                case 'GITHUB_ACCOUNT': {
                    ...
                    try {
                        ...
                        const r = await (await import('axios')).default.get(`https://api.github.com/users/${encodeURIComponent(username)}`, { headers, validateStatus: null });
                        if (r.status !== 200) {
                            passed = false;
                        ...
                        if (req.params?.minRepos && user.public_repos < Number(req.params.minRepos)) {
                            passed = false;
                            message = `Need ${req.params.minRepos}+ public repos (have ${user.public_repos})`;
                            break;
                        }
                        if (req.params?.minFollowers && user.followers < Number(req.params.minFollowers)) {
                            passed = false;
                            message = `Need ${req.params.minFollowers}+ followers (have ${user.followers})`;
                            break;
                        }
                        passed = true;
                        ...
                        passed = false;
                        message = `GitHub check failed: ${e.message}`;
                    }
                    break;
                }
                case 'TELEGRAM_MEMBER': {
                    ...
                    try {
                        const r = await (await import('axios')).default.get(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/getChatMember`, { params: { chat_id: req.params?.chatId, user_id: telegramId } });
                        const status2 = r.data?.result?.status ?? '';
                        ...
                        passed = false;
                        message = `Telegram check failed: ${e.message}`;
                    }
                    break;
                }
                case 'TOKEN_BALANCE': {
                    ...
                    try {
                        ...
                        const result = await checkTokenBalance(req.params?.chainId ?? 1, req.params?.tokenAddress, evmAddress, BigInt(req.params?.minBalance ?? '1'));
                ...
                case 'NFT_OWNERSHIP': {
                    ...
                    try {
                        ...
                        const result = await checkNftOwnership(req.params?.chainId ?? 1, req.params?.contractAddress, evmAddress, req.params?.tokenId);
                        passed = result.passed;
                        message = result.message;
                    }
                    catch (err) {
...
app.post(['/verify/check', '/api/verify/check'], async (req, res) => {
...
async function pinImageToIPFS(file, filename, mimetype) {
    if (!PINATA_JWT) {
...
app.post(['/pin/community', '/api/pin/community'], async (req, res) => {
    const comm = {
    ...
    try {
        ...
        if (idx >= 0) {
            communitiesStore[idx].ipfs_cid = cid;
            persistStore();
        }
...
app.post(['/pin/poll', '/api/pin/poll'], async (req, res) => {
    const poll = {
...
app.post(['/pin/image', '/api/pin/image'], async (req, res) => {
    try {
...
app.post(['/submissions', '/api/submissions'], async (req, res) => {
    const { address, pollId, ciphertext } = req.body;
    if (!address || !pollId || !ciphertext) {
        res.status(400).json({ error: 'address, pollId, ciphertext required' });
        return;
    }
    const addr = address.toLowerCase();
    try {
        if (supabase) {
            await supabase.from('submissions').upsert({ address: addr, poll_id: pollId, ciphertext, saved_at: Date.now() }, { onConflict: 'address,poll_id' });
        }
        else {
            // In-memory fallback
            if (!_memStore[addr])
            ...
            const entry = { pollId, ciphertext, savedAt: Date.now() };
            if (idx >= 0)
                _memStore[addr][idx] = entry;
            else
                _memStore[addr].push(entry);
        }
        res.json({ ok: true });
    }
    catch (err) {
...
app.get(['/submissions/:address', '/api/submissions/:address'], async (req, res) => {
    ...
    try {
        if (supabase) {
            ...
            if (error)
                throw error;
            const rows = (data ?? []).map((r) => ({ pollId: r.poll_id, ciphertext: r.ciphertext, savedAt: r.saved_at }));
            res.json(rows);
        }
        else {
...
// ── GitHub OAuth ──────────────────────────────────────────────────────────────
app.get(['/auth/github', '/api/auth/github'], (_req, res) => {
    const clientId = process.env.GITHUB_CLIENT_ID;
    if (!clientId) {
        res.status(500).send(popupError('eclipse-poll-github', 'GITHUB_CLIENT_ID not configured'));
        return;
    }
...
app.get(['/auth/github/callback', '/api/auth/github/callback'], async (req, res) => {
    ...
    if (error) {
        res.send(popupError('eclipse-poll-github', error));
        return;
    }
    if (!consumeState(state)) {
        res.send(popupError('eclipse-poll-github', 'Invalid or expired OAuth state'));
        return;
    }
    const clientId = process.env.GITHUB_CLIENT_ID;
...
// ── Discord OAuth ─────────────────────────────────────────────────────────────
app.get(['/auth/discord', '/api/auth/discord'], (_req, res) => {
    const clientId = process.env.DISCORD_CLIENT_ID;
    if (!clientId) {
        res.status(500).send(popupError('eclipse-poll-discord', 'DISCORD_CLIENT_ID not configured'));
        return;
    }
    const state = generateState();
...
app.get(['/auth/discord/callback', '/api/auth/discord/callback'], async (req, res) => {
    const { code, state, error } = req.query;
    if (error) {
        res.send(popupError('eclipse-poll-discord', error));
        return;
    }
    if (!consumeState(state)) {
        res.send(popupError('eclipse-poll-discord', 'Invalid or expired OAuth state'));
        return;
    }
    const clientId = process.env.DISCORD_CLIENT_ID;
    ...
    const redirect = `${APP_URL}/auth/discord/callback`;
    try {
        const tokenRes = await axios.post('https://discord.com/api/oauth2/token', new URLSearchParams({ client_id: clientId, client_secret: clientSecret, grant_type: 'authorization_code', code, redirect_uri: redirect }).toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' } });
        const accessToken = tokenRes.data.access_token;
        const expiresIn = tokenRes.data.expires_in ?? 604800;
...
app.get(['/auth/twitter', '/api/auth/twitter'], (_req, res) => {
    const clientId = process.env.TWITTER_CLIENT_ID;
    if (!clientId) {
        res.status(500).send(popupError('eclipse-poll-twitter', 'TWITTER_CLIENT_ID not configured'));
        return;
    }
...
app.get(['/auth/twitter/callback', '/api/auth/twitter/callback'], async (req, res) => {
    const { code, state, error } = req.query;
    if (error) {
        res.send(popupError('eclipse-poll-twitter', error));
        return;
    }
    const entry = consumeState(state);
    if (!entry) {
        res.send(popupError('eclipse-poll-twitter', 'Invalid or expired OAuth state'));
        return;
    }
    const clientId = process.env.TWITTER_CLIENT_ID;
    ...
    try {
        ...
        const { id, username, name } = meRes.data.data;
...
app.get(['/key-backup/:address', '/api/key-backup/:address'], async (req, res) => {
    const addr = req.params.address.toLowerCase();
    try {
        if (supabase) {
            const { data, error } = await supabase
            ...
            res.json({ encrypted_key: data.encrypted_key });
        }
        else {
            const key = _memStore[`__key__${addr}`];
            if (!key) {
                res.status(404).json({ error: 'Not found' });
                return;
            }
