...
import { getPoll, nowSeconds } from '../lib/utils'
...
export default function SurveyDetail() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // Fetch on-chain data
        const onChainPoll = await getPoll(pollId as string)
        ...
        if (onChainPoll?.exists) {
          setIsPollClosed(currentTime > onChainPoll.endBlock)
        }
      } catch (e) {
  ...
  async function handleSubmit() {
    if (!pollId || !allAnswered || !address) return
    ...
    const { fromHex } = await import('../lib/midnight'); const _pib = pollId ? fromHex(pollId.replace(/^0x/,'').padStart(64,'0')) : new Uint8Array(32); await castSurvey(_pib, flat)

    // Save submission so MyVotes can display what was answered
    try {
      const submissionData = {
        pollId: pollId!,
        poll_type: 'survey',
        // Store per-question selected answer index
        surveyAnswers: Object.fromEntries(
          questions.map((_, qi) => [String(qi), answers[qi]])
        ),
        options: questions.map((q, qi) => ({
          id: qi,
      ...
      // 2. Encrypted server copy (non-blocking)
      ;(async () => {
        try {
          const ciphertext = await encryptJSON(submissionData)
    ...
    <div className="max-w-lg mx-auto w-full space-y-4">
      ...
      {isDone && (
        <div className="bg-white rounded-2xl border border-green-100 p-6 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Your answers are ZK-private. Only aggregate results will be revealed.</p>
      ...
      {!isDone && !isPollClosed && (
        <>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <p className="text-xs text-gray-400 mt-2 text-center">Responses are ZK-private. Only aggregate counts are revealed after close.</p>
          </div>
        </>
      )}
