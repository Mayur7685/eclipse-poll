...
import { getBlockHeight } from '../lib/utils'
  ...
  const items: ActivityItem[] = communities.flatMap(c =>
    (c.polls ?? []).flatMap(poll => {
      const events: ActivityItem[] = [{
        ...
        poll, community: c, block: poll.created_at_block ?? 0,
      }]
      // Only mark as ended if we have a current block and end_block is in the past
      if (poll.end_block && currentBlock > 0 && poll.end_block < currentBlock) {
