...
function PollsTab({ polls, currentBlock, communityId, communityName, emptyLabel = 'poll' }: {
  ...
  const now = currentBlock || Math.floor(Date.now() / 1000);
  const activePolls = polls.filter(p => p.end_block ? now <= p.end_block : true)
  const pastPolls   = polls.filter(p => p.end_block ? now > p.end_block : false)
