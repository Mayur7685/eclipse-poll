...
import { deployEclipsePollContract, callCircuitOnMasterContract } from '../lib/eclipse'
import { fromHex } from '../lib/midnight'
import { communityIdFromName } from '../lib/utils'
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1 shadow-sm">
          <button
            type="button"
            onClick={() => set('creator_only', true)}
            className={`flex items-start gap-2.5 p-3 rounded-xl border text-left transition-all ${
              value.creator_only
                ? 'border-[#0070F3] bg-blue-50/50 text-[#0070F3]'
                : 'border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300'
            }`}
          >
            <svg className="w-4 h-4 text-gray-500 mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
            <div>
              <p className="text-xs font-semibold text-gray-900">Creator Only (Strict)</p>
              <p className="text-[11px] text-gray-500 mt-0.5 leading-snug">
                Only community creator can post polls. Recommended for official DAOs.
              </p>
            </div>
          </button>
          ...
          <button
            type="button"
            onClick={() => set('creator_only', false)}
            className={`flex items-start gap-2.5 p-3 rounded-xl border text-left transition-all ${
              !value.creator_only
                ? 'border-[#0070F3] bg-blue-50/50 text-[#0070F3]'
                : 'border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300'
          ...
          >
            <svg className="w-4 h-4 text-blue-500 mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 010 20M12 2a15.3 15.3 0 000 20"/></svg>
            <div>
              <p className="text-xs font-semibold text-gray-900">Open to Members (Community)</p>
              <p className="text-[11px] text-gray-500 mt-0.5 leading-snug">
                Any member holding valid credentials can post polls.
              </p>
            </div>
          </button>
        </div>
    ...
    <div className="bg-gray-50 rounded-xl p-3.5 space-y-2.5 border border-gray-100">
      ...
      {req.type === 'X_FOLLOW' && (
        <Field label="Twitter handle to follow">
          <input className={inputCls} placeholder="e.g. @eclipsepoll" value={req.params.handle ?? ''}
      ...
      {req.type === 'GITHUB_ACCOUNT' && (
        <div className="space-y-2">
          ...
          <Field label="Organization name" optional>
            <input className={inputCls} type="text" placeholder="e.g. eclipsepoll"
          ...
          <Field label="Starred repo (owner/repo)" optional>
            <input className={inputCls} type="text" placeholder="e.g. midnight-ntwrk/midnight"
      ...
      {req.type === 'TELEGRAM_MEMBER' && (
        <>
          <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-700">
            <svg className="w-4 h-4 shrink-0 text-amber-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
  ...
  if (tier.maxGroups === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-10 gap-3 text-center">
        <div className="w-12 h-12 rounded-full bg-green-50 flex items-center justify-center"><svg className="w-6 h-6 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg></div>
...
export default function CreateCommunityWizard() {
  ...
  const { session, address, isConnected, connect } = useWallet()
  ...
  const handleCreate = async () => {
    ...
    if (!activeSession) {
      setError('Please connect your 1AM Wallet extension to deploy & register on Midnight Preprod Network.')
      return
    }
    ...
    try {
      ...
      const config: Partial<CommunityConfig> = {
        ...
        creator:                address || activeSession.unshieldedAddress,
        creator_only:           details.creator_only,
      }

      // 1. ON-CHAIN GUARD RAIL: Execute registerCommunity circuit transaction on Master Contract via 1AM Wallet
      const cIdBytes = fromHex(communityId)
      const dummyBytes = new Uint8Array(32)

      const { txHash, contractAddress } = await callCircuitOnMasterContract(
        activeSession,
        'registerCommunity',
        [cIdBytes, dummyBytes, details.credential_type]
      )

      // 2. ONLY AFTER ON-CHAIN TRANSACTION SUCCEEDS: Pin metadata to IPFS & register in community store
      const cid = await pinCommunityMetadata({ ...config, contract_address: contractAddress })
      await confirmCommunity({ ...config, contract_address: contractAddress, ipfs_cid: cid })
      setCreatedTxHash(txHash || contractAddress)

      setSaving(false)
    } catch (e: unknown) {
      console.error('Community creation guard rail caught error:', e)
      ...
      <div className="max-w-lg mx-auto w-full">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-8 py-10 flex flex-col items-center text-center gap-4">
            ...
            <p className="text-sm text-gray-500 max-w-xs">Your community is registered on Midnight Network and ready to use.</p>
            <a href={createdTxHash && createdTxHash.length === 64
                ? `https://explorer.1am.xyz/tx/${createdTxHash}?network=preprod`
                : `https://explorer.1am.xyz/contract/${import.meta.env.VITE_MIDNIGHT_MASTER_CONTRACT_ADDRESS}?network=preprod`}
              target="_blank" rel="noopener noreferrer"
              className="text-sm font-medium text-[#0070F3] hover:underline">
              View contract on 1AM Explorer ↗
            </a>
