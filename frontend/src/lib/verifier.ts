...
/**
 * Verify requirements and issue Schnorr attestation on Midnight.
 * New flow: verifier issues Schnorr attestation — returns signature for use in ZK circuit.
 * Returns { passed, results, credentialIssued, txHash }.
 */
export const getCredentialParams = async (
  ...
  const raw = await post<CredentialParamsResponse>(
    '/verify/credential-params',
    { communityId, evmAddress, connectedAccounts },
  )
  return raw
}
